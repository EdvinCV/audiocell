...
const ProductoCard = ({productos}) => {
    ...
    const handleAddCarrito = ({id, name, producto, precioVenta, precioVenta2, precioVenta3, color, stock}) => {
        if(name.includes("Kit") || name.includes("Accesorios") || name.includes("Epin")){
            if(id && name && producto && precioVenta && color && stock){
                dispatch(agregarProductoCarrito({id,name,producto,precioVenta,precioVenta2,precioVenta3, color}))
            }
        }else {
                ...
                <tbody>
                {
                    productos.map((prod) => (
                        <tr key={prod.id}>
                            ...
                            <th scope="row">Q{prod.precioVenta}{prod.precioVenta2 ? `-Q${prod.precioVenta2}` : ""}{prod.precioVenta3 ? `-Q${prod.precioVenta3}` : ""}</th>
                            <th scope="row">
                                <button 
